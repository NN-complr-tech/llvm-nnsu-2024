...
public:
	...
	bool VisitVarDecl(clang::VarDecl *varDecl) {
		if (varDecl->getNameAsString() == nameOld_)
			rewriter_.ReplaceText(varDecl->getLocation(), varDecl->getNameAsString().size(), nameNew_);
		if (varDecl->getType().getAsString() == nameOld_ ||
		    varDecl->getType().getAsString() == nameOld_ + " *")
			rewriter_.ReplaceText(varDecl->getTypeSourceInfo()->getTypeLoc().getBeginLoc(),
	...
	bool VisitCXXRecordDecl(clang::CXXRecordDecl *recordDecl) {
		if (nameOld_ == recordDecl->getName()) {
			...
			if (recordDecl->hasUserDeclaredDestructor())
				rewriter_.ReplaceText(destructorDecl->getLocation(), nameOld_.size() + 1, "~" + nameNew_);
		}
		return true;
	}
	...
	bool VisitCXXNewExpr(clang::CXXNewExpr *newExpr) {
		if (nameOld_ == newExpr->getConstructExpr()->getType().getAsString())
			rewriter_.ReplaceText(newExpr->getExprLoc(), nameOld_.size() + 4, "new " + nameNew_);
		return true;
	}
...
public:
	explicit RenameConsumer(clang::CompilerInstance &CI, std::string nameOld, std::string nameNew)
