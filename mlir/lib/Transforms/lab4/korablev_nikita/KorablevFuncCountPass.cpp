...
namespace {
class KorablevFuncCountPass: public PassWrapper<KorablevFuncCountPass, OperationPass<ModuleOp>> {
public:
  StringRef getArgument() const final { return "korablev-func-count-pass"; }
  StringRef getDescription() const final {
    return "A pass that counts the number of times it has been called by other functions in the module";
  }
  ...
  void runOnOperation() override {
    ...
    module.walk([&](LLVM::LLVMFuncOp funcOp) {
      ...
      auto attrValue =
          IntegerAttr::get(IntegerType::get(funcOp.getContext(), 32), callCounter[funcName]);
...
PassPluginLibraryInfo getKorablevFuncCountPassPluginInfo() {
  return {MLIR_PLUGIN_API_VERSION, "korablev-func-count-pass",
          LLVM_VERSION_STRING, []() { PassRegistration<KorablevFuncCountPass>(); }};
}
