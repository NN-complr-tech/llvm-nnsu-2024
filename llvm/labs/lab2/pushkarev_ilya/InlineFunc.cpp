...
llvm::BasicBlock* splitBlockBefore(llvm::BasicBlock* oldBlock,
                                   ...
                                   bool before = true) {
  return llvm::SplitBlock(oldBlock, splitPoint, dtu, li, mssaUpdater, bbName, before);
}

class PushkarevFunctionInliningPass : public llvm::PassInfoMixin<PushkarevFunctionInliningPass> {
public:
    ...
    for (llvm::BasicBlock& block : function) {
      for (llvm::Instruction& instruction : block) {
        ...
        for (llvm::BasicBlock& inlinedBlock : *calledFunction) {
          llvm::BasicBlock* newBlock = llvm::BasicBlock::Create(context, "", &function, nextBlock);
        ...
        for (llvm::BasicBlock& inlinedBlock : *calledFunction) {
          for (llvm::Instruction& inlinedInstruction : inlinedBlock) {
            ...
            if (llvm::isa<llvm::ReturnInst>(&inlinedInstruction)) {
              llvm::BranchInst::Create(&block)->insertInto(currentBlock, currentBlock->end());
            } else {
              ...
              if (llvm::isa<llvm::BranchInst>(*newInstruction)) {
                for (unsigned i = 0; i < newInstruction->getNumOperands(); ++i) {
                  ...
                  if (llvm::isa<llvm::BasicBlock>(*operand)) {
                    newInstruction->setOperand(i, llvm::dyn_cast<llvm::Value>(blockMap[llvm::dyn_cast<llvm::BasicBlock>(operand)]));
                  }
                }
              }
        ...
        splitBlock->getTerminator()->setOperand(0, llvm::dyn_cast<llvm::Value>(blockMap[&calledFunction->getEntryBlock()]));
...
getPushkarevFunctionInliningPassPluginInfo() {
