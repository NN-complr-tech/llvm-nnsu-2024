...
namespace {
struct MuToBitShift : llvm::PassInfoMixin<MuToBitShift> {
public:
...
bool registerPipeline(llvm::StringRef Name, llvm::FunctionPassManager &FPM,
  ...
  if (Name == "MuToBitShift") {
    FPM.addPass(MuToBitShift());
    return true;
  }
  return false;
}

llvm::PassPluginLibraryInfo getMuToBitShiftPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "MuToBitShift", LLVM_VERSION_STRING,
          [](llvm::PassBuilder &PB) {
...
llvmGetPassPluginInfo() {
  return getMuToBitShiftPluginInfo();
}