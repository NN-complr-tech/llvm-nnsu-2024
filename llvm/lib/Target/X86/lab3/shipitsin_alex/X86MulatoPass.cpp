...
namespace {
class X86MulatoPass : public MachineFunctionPass {
public:
  static char ID;

  X86MulatoPass() : MachineFunctionPass(ID) {}

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "X86 Mulato Pass"; }
};
} // namespace

char X86MulatoPass::ID = 0;

bool X86MulatoPass::runOnMachineFunction(MachineFunction &MF) {
  bool changed = false;
